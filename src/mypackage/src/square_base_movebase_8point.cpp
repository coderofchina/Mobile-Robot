...
int main(int argc, char** argv)
{
  ...
  geometry_msgs::Quaternion quaternions[8];

  //convert the angles to quaternions
  double angle = M_PI/2;
  int angle_count = 0;
  for(angle_count = 1; angle_count < 8;angle_count+=2 )
  {
      quaternions[angle_count] = tf::createQuaternionMsgFromRollPitchYaw(0, 0, angle);
      quaternions[angle_count-1] = tf::createQuaternionMsgFromRollPitchYaw(0, 0, angle-(M_PI/2));
      angle = angle + M_PI/2;
  }
  //a pose consisting of a position and orientation in the map frame.
  geometry_msgs::Point point;
  geometry_msgs::Pose pose_list[8];
