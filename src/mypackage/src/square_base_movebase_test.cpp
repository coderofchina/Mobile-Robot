...
//  ROS_INFO("move_base_square.cpp start...");
  ...
  double angle = M_PI/4;
  int angle_count;
  for(angle_count = 0; angle_count < 4;angle_count+=2 )
  {
      quaternions[angle_count] = tf::createQuaternionMsgFromRollPitchYaw(0, 0, angle);
  }
  angle = -M_PI/4;
  for(angle_count = 1; angle_count < 4;angle_count+=2 )
  {
      quaternions[angle_count] = tf::createQuaternionMsgFromRollPitchYaw(0, 0, angle);
  }
...
//////
     geometry_msgs::Pose pose_list2[3];
     geometry_msgs::Pose pose_label;
double origin_x=transform.getOrigin().x();
double origin_y=transform.getOrigin().y();
double min_distance;
for(int aa = 0;aa<3;aa++)
{
if(origin_x!=pose_list[aa].position.x || origin_y!=pose_list[aa].position.y)
  {pose_list2[aa].position.x==pose_list[aa].position.x;
  pose_list2[aa].position.y==pose_list[aa].position.y;
  }
}

min_distance = sqrt(pow((fabs(origin_x - pose_list2[0].position.x)), 2) +  pow((fabs(origin_y - pose_list2[0].position.y)), 2));
pose_label.position.x = pose_list2[0].position.x;
pose_label.position.y = pose_list2[0].position.y;
for(int bb = 1;bb<3;bb++)
{

if( (  sqrt(pow((fabs(origin_x - pose_list2[bb].position.x)), 2) +  pow((fabs(origin_y - pose_list2[bb].position.y)), 2))  )  < min_distance)
   {
     min_distance = sqrt(pow((fabs(origin_x - pose_list2[bb].position.x)), 2) +  pow((fabs(origin_y - pose_list2[bb].position.y)), 2));
     pose_label.position.x = pose_list2[bb].position.x;
     pose_label.position.y = pose_list2[bb].position.y;
    }
}
////////
     ...
     goal.target_pose.pose = pose_label;
